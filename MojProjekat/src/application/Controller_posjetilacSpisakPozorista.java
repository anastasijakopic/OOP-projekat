...
public class Controller_posjetilacSpisakPozorista implements Initializable{
    ...
    @FXML
    private ChoiceBox<Predstava> listaPredstava;
    ...
    void klikScena(ActionEvent event) {
    	try {
			...
			stage.setTitle("Pogledajte sva ostala izvođenja predstava u svim pozorištima! ");
    ...
    void potvrdaPozorista(ActionEvent event) {
    	...
    	if(listaPozoriste.getSelectionModel().getSelectedItem() == null) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	...
    	 	alert0.setContentText("NEOPHPDNO JE DA IZABERETE POZORIŠTE!");
    	 	alert0.show();
    	}
    	else {
    	...
    	labela.setText(selectedPozoriste.getNaziv() + " pozorištu.");
		...
		listaPredstava.getItems().addAll(pomocnaListaPredstava);
		listaPredstava.setVisible(true);
		if(listaPredstava.getItems().isEmpty()) {
			Alert alert4 = new Alert(Alert.AlertType.INFORMATION);
			alert4.setTitle("OBAVJESTENJE!");
			alert4.setContentText("U pozorištu koje ste odabrali nema izvođenja predstava u narednom periodu.\nMolimo Vas da izaberete drugo pozorište!");
			alert4.show();
		}
    	}
    }
    ...
    void potvrdaPredstava(ActionEvent event) {
    	...
    	else if(listaPredstava.getValue()==null) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("NEOPHODNO JE DA IZABERETE PREDSTAVU!");
    	 	alert0.show();
    	}
    	else {
		...
		Predstava predstava = listaPredstava.getValue();
    ...
    void potvrdaRezervacije(ActionEvent event) {
    	if(choiceBoxTermini.getValue() == null && textFieldBrojKarata.getText().equals("")) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("NEOPHODNO JE DA IZABERETE TERMIN IZVOĐENJA, KOJI VAM NAJVIŠE ODGOVARA!\nTAKOĐE, NEOPHODNO JE DA UNESETE KOLIKO KARATA ŽELITE DA REZERVIŠETE!");
    	 	alert0.show();
    	}
    	else if(choiceBoxTermini.getValue() == null){
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("NEOPHPDNO JE DA IZABERETE TERMIN IZVOĐENJA, KOJI VAM NAJVIŠE ODGOVARA");
    	 	alert0.show();
    	}
    	else if(textFieldBrojKarata.getText().equals("") || Integer.parseInt(textFieldBrojKarata.getText()) <= 0) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("UNESITE BROJ, KOJI PREDSTAVLJA BROJ KARATA KOJI ĆETE REZERVISATI!");
    	 	alert0.show();
    	}
    		...
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	...
    	 	alert0.setTitle("UPOZORENJE");
    	 	String s = "Slobodan broj sjedišta je " + Pozoriste.dajBrojSlobodnihSjedista(listaPozoriste.getSelectionModel().getSelectedItem(), choiceBoxTermini.getValue())+", a vama je potrebno " + textFieldBrojKarata.getText() +
    	 			"\nDakle, za odabrani termin nije moguće izvršiti rezervaciju. Molimo Vas da izaberete drugi termin!";
    		...
    		Alert alert0 = new Alert(Alert.AlertType.CONFIRMATION);
		 	...
		 	if(Controller_posjetilac.prijava) {
		 	 korisnicko = Controller_posjetilac.korisnickoPosjetioicaPrijava;
		 		 ...
		 		 korisnicko=Controller_posjetilacRegistracija.korisnickoPosjetioicaRegistracija;
		 	}
			...
			String s = "Rezime informacija." + imePosj + ", izabrali ste:\n";
		 	s += "Pozorište: " + listaPozoriste.getSelectionModel().getSelectedItem().getNaziv() + " u " + listaPozoriste.getSelectionModel().getSelectedItem().getGrad() + ".\n";
		 	s += "Predstava: " + listaPredstava.getValue().getNaziv() + ", zanr: " + listaPredstava.getValue().getStringZanr(listaPredstava.getValue().getZanr()) + ".\n";
		 	s += "Termin izvođenja predstave: " + choiceBoxTermini.getValue()+"\n";
		 	...
		 	if(result.get()==ButtonType.OK) {
		    	...
		    	if(posmatranje) {
		    		Alert alert4 = new Alert(Alert.AlertType.INFORMATION);
	        		alert4.setTitle("OBAVJESTENJE!");
	        		alert4.setContentText("Uspješno ste rezervisali kartu!");
    	...
    	if(choiceBoxTermini.getValue() == null) {
    		...
    		if(checkBoxBrojKarata.isSelected()) {
    			...
    			if(brojSjedistaUPoz == 0) {
    				...
    				nemaslkarata.setText("Nazalost, nema slobodnih mjesta u terminu: " + ipSelected.toString());
    			}
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		listaPredstava.setVisible(false);
		...
		labelaBrojKarata.setVisible(false);
		nemaslkarata.setVisible(false);
	}
