...
public class Controller_posjetilac implements Initializable{
    ...
    void prijaviSe(ActionEvent event) {
    	if(tf_koriscnicko.getText().equals("") && tf_lozinka.getText().equals("")) {
			...
			alert3.setContentText("Prvo unesite korisnčiko ime!");
			alert3.show();
    	}
    	else if(tf_lozinka.getText().equals("")) {
    		Alert alert2 = new Alert(Alert.AlertType.ERROR);
			alert2.setContentText("Unesite lozinku za: " +  " korisničko ime " + tf_koriscnicko.getText() );
			alert2.show();
    	}
    	...
    	for(Posjetilac_pozorista pp : Posjetilac_pozorista.getPosjetilac_pozorista_lista()) {
    		if(pp.getKorisnicko_ime().equals(tf_koriscnicko.getText())) {
    			...
    			if(!pp.getLozinka().equals(Posjetilac_pozorista.getMd5(tf_lozinka.getText()))) {
    				tf_lozinka.setText("");
 		    	 	...
 		    	 	alert0.setContentText("Neipsravna lozinka za " + tf_koriscnicko.getText() + ".Pokušajte ponovo");
 		    	 	alert0.show();
					}
    			else {
    				connect = Konekcija.getConnection();
    		    	String sql = "SELECT * FROM posjetilac_pozorista WHERE korisincko_ime = ? AND lozinka = ?";
    		    	try {
    		...
    		if(!postojiPosjetilac) {
				 Alert alert0 = new Alert(Alert.AlertType.WARNING);
		    	 	alert0.setTitle("UPOZORENJE");
		    	 	alert0.setContentText("Korisničko ime: " + tf_koriscnicko.getText() + " ne postoji. Pokušajte ponovo");
		    	 	alert0.show();
    ...
    @FXML
    void switchtoPosjetilacRegistracija(ActionEvent event) {
    	try {
			prijava = false;
			Parent root = FXMLLoader.load(getClass().getResource("Scena_posjetilacRegistracija.fxml"));
			Stage stage = (Stage)((Node)event.getSource()).getScene().getWindow();
			Scene scene = new Scene(root);
			stage.setTitle("Dobrodošli posjetioče, registrujte se");
			stage.setScene(scene);
			stage.show();
		}
		catch(IOException e) {
