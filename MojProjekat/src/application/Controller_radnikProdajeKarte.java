...
public class Controller_radnikProdajeKarte implements Initializable{
    ...
    void prodajaKarte(ActionEvent event) {
    	if(choiceBoxTermini.getValue() == null && textFieldBrojKarata.getText().equals("")) {
    		...
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("ODABERITE JEDNU OD OPCIJA, DA LI POSJETILAC VEĆ IMA NALOG ILI NEMA");
    	 	alert0.show();
    	}
    	else if(imaNalogID.isSelected() && (korisnickoImeImatf.getText().equals("") || lozinkaImatf.getText().equals(""))) {
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	alert0.setContentText("NEKA POSJETILAC UNESE SVOJE KORISCNIČKO IME I LOZINKU");
    	 	alert0.show();
    	}
    		...
    		Alert alert0 = new Alert(Alert.AlertType.WARNING);
    	 	alert0.setTitle("UPOZORENJE");
    	 	String s = "Slobodan broj sjedišta je " +Pozoriste.dajBrojSlobodnihSjedista(Pozoriste.dajPozoristeNaOsnovuId(Controller_nakonPrijaveRadnika.vratiPozoristeID()), choiceBoxTermini.getValue()) +", a vama je potrebno " + textFieldBrojKarata.getText() + 
    		...
    		if(imaNalogID.isSelected()) {
    			...
    			for(Posjetilac_pozorista pp : Posjetilac_pozorista.getPosjetilac_pozorista_lista()) {
    				if(pp.getKorisnicko_ime().equals(korisnickoImeImatf.getText())) {
    					...
    					if(!pp.getLozinka().equals(Posjetilac_pozorista.getMd5(lozinkaImatf.getText()))) {
    						...
    						Alert alert0 = new Alert(Alert.AlertType.WARNING);
        		    	 	alert0.setTitle("UPOZORENJE");
        		    	 	alert0.setContentText("Neispravna lozinka za " + korisnickoImeImatf.getText() + " .Pokušajte ponovo");
        		    	 	alert0.show();
    					}
    					else {
    						uslov1=true;
    						...
    						for(Karta k : Karta.getKarta_lista()) {
    							if(k.getIzvodjenje_predstave_ID() == choiceBoxTermini.getValue().getID())
    								if(k.getPosjetilac_ID() == pp.getID() && k.getStatus() == 2) {
	    								...
	    								Alert alert0 = new Alert(Alert.AlertType.WARNING);
	    	        		    	 	alert0.setTitle("UPOZORENJE");
	    	        		    	 	alert0.setContentText("Posjetioče " + pp.getIme() + ", Vi ste već rezervisali kartu za ovo izvođenje i niste je preuzeli, te nije moguće da vam je prodamo!");
	    	        		    	 	alert0.show();
    							}
    			...
    			if(postojiLiPosjetilac && uslov && uslov1) {
						...
						//REZIME INFORMACIJA
					 	...
					 	s+= "Pozorište: " + Pozoriste.dajPozoristeNaOsnovuId(Controller_nakonPrijaveRadnika.vratiPozoristeID());
					 	...
					 	if(result.get()==ButtonType.OK) {
					 		...
					 		if(posmatranje) {
				        		...
				        		alert4.setContentText("Uspješno ste prodali kartu!");
				        		Optional <ButtonType>result1 = alert4.showAndWait();
				    		 	if(result1.get()==ButtonType.OK) {
				    		 		try {
				    					...
				    					stage.setTitle("Radniče " + Controller_radnik.vratiImePrezime()  + ", upravljajte prodajom/rezervacijama karata!");
    			 ...
    			 if(!postojiLiPosjetilac) {
    				...
    				Alert alert0 = new Alert(Alert.AlertType.WARNING);
		    	 	alert0.setTitle("UPOZORENJE");
		    	 	alert0.setContentText("Koriscničko ime " + korisnickoImeImatf.getText() + " ne postoji!");
		    	 	alert0.show();
		    	 	korisnickoImeImatf.setText("");
		    	 	lozinkaImatf.setText("");
    			}
    		...
    		if(nemaNalogID.isSelected()) {
    			...
    			if(postojiKorisnicko) {
    				Alert alert0 = new Alert(Alert.AlertType.WARNING);
		    	 	alert0.setTitle("UPOZORENJE");
		    	 	alert0.setContentText("Korisničko ime " + korisnickoImeNematf.getText() + " već postoji. Molimo Vas da unesete drugo!");
		    	 	alert0.show();
		    	 	korisnickoImeNematf.setText("");
		    	 	imeNematf.setText("");
		    	 	prezimeNematf.setText("");
