...
public class Controller_posjetilacRegistracija implements Initializable{
    ...
    @FXML
    void regirstrujSe(ActionEvent event) {
    	if(tf_koriscnicko.getText().equals("") && tf_lozinka.getText().equals("") && tf_ime.getText().equals("") && tf_prezime.getText().equals("")) {
    		...
    		if(postojiKorisnicko) {
    			Alert alert0 = new Alert(Alert.AlertType.WARNING);
	    	 	...
	    	 	alert0.setContentText("Korisničko ime " + tf_koriscnicko.getText() + " već postoji. Molimo Vas da unesete drugo!");
	    	 	alert0.show();
				...
				try {
					statement = connect.prepareStatement("INSERT INTO posjetilac_pozorista (ime, prezime, korisincko_ime, lozinka) VALUE (?,?,?,?)");
    				...
    				stage.setTitle("Dobrodošli posjetioče, " + Controller_posjetilacRegistracija.vratiImePrezimePosjetiocaRegistracija());
    ...
    @FXML
    void switchtoPosjetilacPrijava(ActionEvent event) {
    	try {
			Parent root = FXMLLoader.load(getClass().getResource("Scena_posjetilac.fxml"));
			Stage stage = (Stage)((Node)event.getSource()).getScene().getWindow();
			Scene scene = new Scene(root);
			stage.setTitle("Dobrodošli! Ukoliko imate nalog, registujte se!");
			stage.setScene(scene);
			stage.show();
		}
		catch(IOException e) {
