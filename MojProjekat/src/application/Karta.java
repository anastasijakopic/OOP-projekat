...
public class Karta {
	...
	public String toString() {
		...
		if(this.status == 1) 
			string += "Status: kupljena, ne koristeći ovu aplikaciju\n";
		else if(this.status == 2)
			string += "Status: rezervisana koristeći aplikaciju, ali nije preuzeta\n";
		else
			string += "Status: rezervisana koristeći aplikaciju, preuzeta\n";
		...
		for(Izvodjenje_predstave ip : Izvodjenje_predstave.getLista_izvodjenje_predstave()) {
			if(this.getIzvodjenje_predstave_ID() == ip.getID()) {
				...
				string+= "Ime pozorišta: " + Pozoriste.dajPozoristeNaOsnovuId(ip.getPozoriste_ID()).getNaziv()+"\nGrad: " + Pozoriste.dajPozoristeNaOsnovuId(ip.getPozoriste_ID()).getGrad()+"\n";
				string+="Cijena: " + ip.getCijena() + "\n";
				string+="Datum izvođenja: " + ip;	
			}
