...
public class Controller_radnikKreiraRadnika implements Initializable{
    ...
    void regirstrujSe(ActionEvent event) {
    	if(tf_korisnicko.getText().equals("") && tf_lozinka.getText().equals("") && tf_ime.getText().equals("") && tf_prezime.getText().equals("")) {
    		...
    		alert5.setContentText("Neophodno je da izaberete pozorište");
    		alert5.show();
    	}
	    		...
	    		alert6.setContentText("Ukoliko želite da dodate novog radnika u pozorištu u kojem NE radite neophodno je da izaberete pozorište u kome nema radnika\n" + choiceBox.getValue().getNaziv() + " pozorište već posjeduje radnika.");
	    		alert6.show();			
    	}
    	...
    	try {
    			...
    			if(result.isBeforeFirst()) {
    				...
    				alert.setContentText("Korisničko ime " + tf_korisnicko.getText() + " već postoji. Molimo Vas unesite novo korisničko ime! ");
    				alert.show();
	        		...
	        		alert4.setContentText("Uspješno ste dodali radnika!");
