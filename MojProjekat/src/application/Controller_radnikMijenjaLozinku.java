...
public class Controller_radnikMijenjaLozinku implements Initializable{
    ...
    @FXML
    void potvrdaPromjeneLozinke(ActionEvent event) {
    	Alert alert0 = new Alert(Alert.AlertType.CONFIRMATION);
	 	alert0.setTitle("POTVRDA");
	 	alert0.setContentText("Da li ste sigurni da želite da promijenite lozinku?");
    	...
    	if(tf_trenutna.getText().equals("") && tf_nova.getText().equals("")) {
    		...
    		Alert alert3 = new Alert(Alert.AlertType.ERROR);
			alert3.setContentText("Netacno unesena lozinka za korisničko ime:\n" + Controller_radnik.vratiKorisnicko());
			tf_trenutna.setText("");
			tf_nova.setText("");
			alert3.show();
    		...
    		Alert alert3 = new Alert(Alert.AlertType.ERROR);
			alert3.setContentText("Koriscniče " + Controller_radnik.vratiKorisnicko() + " unijeli ste opet staru lozinku za novu. Ako želite da promijenite šifru unesite potpuno novu šifru!");
			alert3.show();
			tf_nova.setText("");
    	}
        	...
        	try {
        		...
        		alert4.setContentText("Uspješno ste promijenili lozinku!");
